...
// CreateAlertRuleRequest is the request for creating an alert rule
type CreateAlertRuleRequest struct {
	Name        string                 `json:"name" binding:"required"`
	Type        string                 `json:"type" binding:"required"` // budget_threshold, goal_milestone, scheduled_report, etc.
	Enabled     bool                   `json:"enabled"`                 // default: true
	Description string                 `json:"description"`
	Conditions  map[string]interface{} `json:"conditions" binding:"required"` // rule-specific conditions
	Channels    []string               `json:"channels"`                      // channels to use for this rule
	Schedule    *string                `json:"schedule"`                      // cron expression (optional)
}

// UpdateAlertRuleRequest is the request for updating an alert rule
...
// ListAlertRulesRequest is the request for listing alert rules
type ListAlertRulesRequest struct {
	Type    string `form:"type"`    // filter by rule type
	Enabled *bool  `form:"enabled"` // filter by enabled status
	Limit   int    `form:"limit" binding:"omitempty,min=1,max=100"`
	Offset  int    `form:"offset" binding:"omitempty,min=0"`
}

// GetAnalyticsRequest is the request for getting analytics data
